# The service accounts come from infra/bootstrap. They are looked up by name
# rather than read out of bootstrap's state on purpose: a
# `terraform_remote_state` data source would mean this stack needs credentials
# for bootstrap's backend, and would couple the two states so a bootstrap
# refactor could break an apply here. An identity's email is a stable contract;
# a state file's internal shape is not.
